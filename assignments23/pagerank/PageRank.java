...
public class PageRank {
	...
	void iterate(int numberOfDocs, int maxIterations, Map<String, Boolean> options) {
		...
		if (options.get("power_iteration"))
			...
			x = this.monte_carlo_1(numberOfDocs, maxIterations);
		else if (options.get("monte_carlo_2"))
			x = this.monte_carlo_2(numberOfDocs, maxIterations);
		else if (options.get("monte_carlo_4"))
			x = this.monte_carlo_4(numberOfDocs, maxIterations);
		else if (options.get("monte_carlo_5"))
			this.monte_carlo_5(numberOfDocs, maxIterations);

		// Sort and print top 30 docs
		ArrayList<Document> docs = new ArrayList<Document>();
		for (int i = 0; i < numberOfDocs; i++)
			docs.add(new Document(docName[i], x[i]));
		Collections.sort(docs);
		for (int i = 0; i < 30; i++)
			print(String.valueOf(i + 1) + ": " + docs.get(i).docNumber + " " + String.valueOf(docs.get(i).docScore));
	...
	private double[] power_iteration(int numberOfDocs, int maxIterations) {
		...
		while (iter < maxIterations) {
			...
			Arrays.fill(x_new, 0.506515 / numberOfDocs); // 1.0
			for (int i = 0; i < numberOfDocs; i++) {
				if (link.get(i) != null) {
	...
	private double[] monte_carlo_1(int numberOfDocs, int maxIterations) {
		...
		for (int iter = 0; iter < maxIterations; iter++) {
			if (iter > next_target * maxIterations) {
